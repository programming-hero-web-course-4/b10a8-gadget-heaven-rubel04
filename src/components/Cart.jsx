      ...
      <div className="w-7xl mx-auto">
        <div className="flex justify-between items-center py-6">
          ...
          <div className="flex gap-6">
            ...
            <button onClick={handleSortProduct} className="text-[#9538E2] flex items-center gap-1 cursor-pointer font-bold border-2 border-[#9538E2] bg-white py-1 px-8 rounded-full">
              Sort By Price
              <BsSortNumericUpAlt></BsSortNumericUpAlt>
            </button>
            <button onClick={handlePurchase} className="bg-[#9538E2] font-bold border-2 cursor-pointer border-[#9538E2] text-white py-1 px-8 rounded-full">
              Purchase
            </button>
          </div>
        </div>
        {
            carts.length !== 0? <div className="space-y-4">
            {carts.map((cart, idx) => (
              <div
                key={idx}
              ...
              >
                ...
                <div>
                  <button onClick={()=> handleDelete(cart.product_id)} className=" text-red-300 cursor-pointer md:mr-10"><IoIosCloseCircleOutline className="text-3xl"></IoIosCloseCircleOutline></button>
                </div>
              </div>
            ))}
          </div>
          :
          <div className="flex justify-center"><img src="https://res.cloudinary.com/daxjoqam6/image/upload/v1737840470/no-product_ebgqos.png" alt="" /></div>
        }
      </div>
    </div>
  );
